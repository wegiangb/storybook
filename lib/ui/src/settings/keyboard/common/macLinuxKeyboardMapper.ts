...
import { ScanCode, ScanCodeUtils, IMMUTABLE_CODE_TO_KEY_CODE, IMMUTABLE_KEY_CODE_TO_CODE, ScanCodeBinding } from './scanCode';
...
import { ChordKeybinding } from 'lib/ui/src/keyboard/keyCodes';
...
const macLinuxKeyMappingEquals = (a: IMacLinuxKeyMapping, b: IMacLinuxKeyMapping): boolean => {
    if (!a && !b) {
        return true;
    }
    if (!a || !b) {
...
export const macLinuxKeyboardMappingEquals = (a: IMacLinuxKeyboardMapping, b: IMacLinuxKeyboardMapping): boolean => {
    if (!a && !b) {
        return true;
    }
...
export class NativeResolvedKeybinding extends ResolvedKeybinding {
    ...
    public getLabel(): string {
        const firstPart = this._mapper.getUILabelForScanCodeBinding(this._firstPart);
        const chordPart = this._mapper.getUILabelForScanCodeBinding(this._chordPart);
        return `${firstPart} ${chordPart}`
        // return UILabelProvider.toLabel(this._firstPart, firstPart, this._chordPart, chordPart, this._OS);
    }

    public getUserSettingsLabel(): string {
        const firstPart = this._mapper.getUserSettingsLabelForScanCodeBinding(this._firstPart);
        const chordPart = this._mapper.getUserSettingsLabelForScanCodeBinding(this._chordPart);
    ...
    public getDispatchParts(): [string, string] {
        const firstPart = this._firstPart ? this._mapper.getDispatchStrForScanCodeBinding(this._firstPart) : null;
        const chordPart = this._chordPart ? this._mapper.getDispatchStrForScanCodeBinding(this._chordPart) : null;
        return [firstPart, chordPart];
    }
...
class ScanCodeCombo {
    ...
    public toString(): string {
        return `${this.ctrlKey ? 'Ctrl+' : ''}${this.shiftKey ? 'Shift+' : ''}${this.altKey ? 'Alt+' : ''}${ScanCodeUtils.toString(
            this.scanCode
        )}`;
    }
    ...
    public equals(other: ScanCodeCombo): boolean {
        return (
            this.ctrlKey === other.ctrlKey && this.shiftKey === other.shiftKey && this.altKey === other.altKey && this.scanCode === other.scanCode
        );
    }
...
class KeyCodeCombo {
    ...
    public toString(): string {
        return `${this.ctrlKey ? 'Ctrl+' : ''}${this.shiftKey ? 'Shift+' : ''}${this.altKey ? 'Alt+' : ''}${KeyCodeUtils.toString(
            this.keyCode
        )}`;
    }
}
...
class ScanCodeKeyCodeMapper {
     ...
     * Only covers relevant modifiers; ctrl, shift, alt(sdince meta does not influence mappings)
     */
    ...
    public registerIfUnknown(scanCodeCombo: ScanCodeCombo, KeyCodeCombo: KeyCodeCombo): void {
        ...
        if (keyCodeIsDigit || keyCodeIsLetter) {
            // Only check that we don't inser the same entry twice
...
export class MacLinuxKeyboardMapper implements IKeyboardMapper {
        ...
        for (let scanCode = ScanCode.None; scanCode < ScanCode.MAX_VALUE; scanCode++) {
            ...
            for (let mod = 0; mod < 8; mod++) {
                ...
                if (kbCombos.length === 0) {
                    result.push(
                        `| ${this._leftPad(outScanCodeCombo, 30)} | ${outKey} | ${this._leftPad('', 25)} | ${this._leftPad('', 3)} | ${this._leftPad(
                            `${outUILabel}`,
                            25
                        )} | ${this._leftPad(outUserSettings, 30)} | ${this._leftPad(outElectronAccelerator, 25)} | ${this._leftPad(
                            outDispatchStr,
                            30
                        )} | ${outWYSIWYG} |`
                    );
                    ...
                    for (let i = 0, len = kbCombos.length; i < len; i++) {
                        ...
                        if (i === 0) {
                            result.push(
                                `| ${this._leftPad(outScanCodeCombo, 30)} | ${outKey} | ${this._leftPad(outKeybinding, 25)} | ${this._leftPad(
                                    colPriority,
                                    3
                                )} | ${this._leftPad(`${outUILabel}`, 25)} | ${this._leftPad(outUserSettings, 30)} | ${this._leftPad(
                            ...
                            result.push(
                                `| ${this._leftPad('', 30)} |       | ${this._leftPad(outKeybinding, 25)} | ${this._leftPad(
                                    colPriority,
                                    3
                                )} | ${this._leftPad('', 25)} | ${this._leftPad('', 30)} | ${this._leftPad('', 25)} | ${this._leftPad('', 30)} |         |`
                            );
                        }
                    }
                }
    ...
    public getUserSettingsLabelForScanCodeBinding(binding: ScanCodeBinding): string {
        ...
        if (constantKeyCode !== -1) {
            ...
            const reverseBindings = this.simpleKeybindingToScanCodeBinding(
                new SimpleKeybinding(binding.ctrlKey, binding.shiftKey, binding.altKey, binding.metaKey, constantKeyCode, '')
            );
