...
import { IMMUTABLE_CODE_TO_KEY_CODE, ScanCode, ScanCodeBinding, ScanCodeUtils } from './scanCode';
...
export class WindowsNativeResolvedKeybinding extends ResolvedKeybinding {
  ...
  public getUSLabel(): string | null {
    const firstPart = this._getUSLabelForKeybinding(this._firstPart);
    const chordPart = this._getUSLabelForKeybinding(this._chordPart);
    return UILabelProvider.toLabel(this._firstPart, firstPart, this._chordPart, chordPart, OperatingSystem.Windows);
  }
  ...
  public getAriaLabel(): string | null {
    ...
    return `first part: ${firstPart} chord part: ${chordPart}`;
  }
  ...
  public getUserSettingsLabel(): string | null {
    ...
    const result = UserSettingsLabelProvider.toLabel(this._firstPart, firstPart, this._chordPart, chordPart, OperatingSystem.Windows);
  ...
  public getDispatchParts(): [string | null, string | null] {
    const firstPart = this._firstPart ? this._getDispatchStr(this._firstPart) : null;
    const chordPart = this._chordPart ? this._getDispatchStr(this._chordPart) : null;
    return [firstPart, chordPart];
  }
  ...
  public dumpDebugInfo(): string {
    ...
    for (let scanCode = ScanCode.None; scanCode < ScanCode.MAX_VALUE; scanCode++) {
      ...
      for (const mod of mods) {
        ...
        result.push(
          `| ${this._leftPad(outScanCode, 30)} | ${outKey} | ${this._leftPad(outKb, 25)} | ${this._leftPad(
            outUILabel,
            25
          )} |  ${this._leftPad(outUserSettings, 25)} | ${outWYSIWYG} |`
        );
      }
      result.push(
        `-----------------------------------------------------------------------------------------------------------------------------------------`
