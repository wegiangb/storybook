...
const extractKeyCode = (e: KeyboardEvent): KeyCode => {
  if (e.charCode) {
...
export const getCodeForKeyCode = (keyCode: KeyCode): number => INVERSE_KEY_CODE_MAP[keyCode];
function prop<T, K extends keyof T>(obj: T, key: K) {
  return obj[key];
}
export interface IKeyboardEvent<T> {
...
interface SyntheticEvent extends KeyboardEvent {
  nativeEvent: KeyboardEvent;
}

export class StandardKeyboardEvent implements IKeyboardEvent<StandardKeyboardEvent> {
  ...
  constructor(source: SyntheticEvent) {
    const e = source;
    const { nativeEvent } = e;
    ...
    this.code = code;
    const k = getCodeForKeyCode(this.keyCode);
    console.log(k);
    console.info(e.type);
    console.info(': keyCode: ' + e.keyCode);
    console.info('charCode: ' + e.charCode);
    console.info('keycosde: ' + k);
    // console.info(e.type + + ", which: " + e.which + ", charCode: " + e.charCode + ", detail: " + e.detail + " ====> " + this.keyCode + ' -- ' + KeyCode[k]);
  ...
  private _computeRuntimeKeybinding(): SimpleKeybinding {
    ...
    return new SimpleKeybinding(this.ctrlKey, this.shiftKey, this.altKey, this.metaKey, this.keyCode);
  }
}
