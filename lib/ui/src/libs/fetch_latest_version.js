...
export default function fetchLatestVersion(currentVersion) {
  const time = Date.now();
  return Promise.resolve(JSON.parse(localStorage.getItem('lastUpdateCheck'))).then(
    (fromCache = { success: false, time: 0 }) => {
      ...
      if (!fromCache || time - fromCache.time > checkInterval) {
        return Promise.race([
          ...
          .then(data => data.json())
          .then(result =>
            Promise.resolve(localStorage.setItem('lastUpdateCheck', JSON.stringify(result))).then(() =>
              Promise.resolve({ success: true, result, time })
            )
          );
      }
